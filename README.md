# Lidar-Defect-Viewer-
An FYP web platform for interactive 3D defect reporting, built by processing LiDAR (IFC) scan data and rendering it with Babylon.js.
...
Modern tools like Metaroom can capture 3D LiDAR scans and "Snapshot" images of defects. However, a significant gap exists: this captured data (like `.ifc` files) often severs the link between the defect image and its precise 3D coordinate, requiring specialized software or manual cross-referencing to review.

## 2. Project Objectives

This project aims to build an automated pipeline and web dashboard to solve this problem.
...
3.  **To build** an intuitive web dashboard (using `Babylon.js`) that displays the 3D model overlaid with interactive visual markers at each defect's location.
